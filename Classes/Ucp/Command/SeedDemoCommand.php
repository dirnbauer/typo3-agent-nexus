...
namespace Webconsulting\AgentNexus\Ucp\Command;
...
/**
 * Seed the UCP Agent Checkout demo plugin onto a page. Idempotent:
 * re-running refreshes the existing element (matched by CType + page).
 */
#[AsCommand(
    name: 'ucp:seed:demo',
    description: 'Place (or refresh) the UCP Agent Checkout plugin on a page. Idempotent.',
)]
final class SeedDemoCommand extends Command
{
    private const CTYPE = 'agentnexus_checkout';
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addOption('sorting', null, InputOption::VALUE_REQUIRED, 'Sorting value', '3000')
            ->addOption('header', null, InputOption::VALUE_REQUIRED, 'Headline', 'Let an agent set up your order');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($existing !== false) {
            ...
            $io->success(sprintf('Refreshed UCP Agent Checkout plugin (uid %d) on page %d.', (int)$existing, $page));
        ...
        $io->success(sprintf('Created UCP Agent Checkout plugin (uid %d) on page %d.', $uid, $page));
